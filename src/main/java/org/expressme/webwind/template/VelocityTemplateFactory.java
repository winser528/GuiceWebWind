...
public class VelocityTemplateFactory extends TemplateFactory {
    ...
    public Template loadTemplate(String path) throws Exception {
        if (log.isDebugEnabled()) log.debug("Load Velocity template '" + path + "'.");
    ...
    public void init(Config config) {
        ...
        if (!webAppPath.endsWith("/") && !webAppPath.endsWith("\\")) webAppPath = webAppPath + File.separator;
        log.info("Detect web application path: " + webAppPath);
        log.info("init VelocityTemplateFactory...");
        rtInstance = new RuntimeInstance();
        // read property file:
        Properties props = readProperties(webAppPath + "/velocity.properties");
        checkProperty(props);
